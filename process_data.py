...
def clean_amount(amount_str):
    ...
    if amount_str is None:
        return 0.0
    # More robust cleaning: remove all non-digit characters except the decimal point
...
# --- REFINED REGEX PATTERNS FOR SMS PARSING ---
# These patterns are now more flexible to handle variations.
PATTERNS = {
    ...
    # Adding a pattern for OTPs to explicitly ignore them
...
def parse_sms(sms_node):
    ...
    for type, pattern in PATTERNS.items():
        ...
        if match:
            # Explicitly ignore OTP messages
            if type == 'otp':
                return None
    ...
    # If no pattern matched, log it as unprocessed
